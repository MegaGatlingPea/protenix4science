...
def make_dummy_feature(
    ...
    if "msa" in dummy_feats:
        ...
        # [Xujun] START 把35维的restype转换为32维的restype
...
def convert_bb_configs(config_file_path, dump=False, output_path=None):
    ...
    Args:
        config_file_path (str): bb generation configuration file path
        dump (bool): whether to save output to file, default is False
        output_path (str, optional): output file path, if dump=True and not specified, use default path
    
    Returns:
        List[Dict]: output_json
            - output_json: converted configuration list
    """
    output_json = []
