...
class MultiChainPermutation(object):
    ...
    def process_input(
        ...
        if len(torch.unique(label_full_dict["entity_mol_id"])) == len(
            ...
            # [Xujun] START 如果存在的chain数量小于等于masked的chain数量，则不进行chain permutation
            torch.unique(label_full_dict["mol_id"]).size(0) <= num_masked_asym_ids
            # [Xujun] END
            ):
    ...
    def find_anchor_asym_chain_in_predictions(self) -> tuple[int]:
        ...
        # [Xujun] START 在计算chain permutation时，去除masked的chain    
        valid_asyms = [asym_id for asym_id, l in asym_to_asym_length.items() if l >= 4 and asym_id not in self.masked_asym_ids]
        # [Xujun] END
        ...
        # [Xujun] START 在计算chain permutation时，去除masked的chain，可能会造成entity_to_asym_count为空
    ...
    def compute_best_match_heuristic(self):
        ...
        # [Xujun] START 在计算chain permutation时，去除masked的chain，可能会造成anchor_pred_asym_id为None
        ...
        for anchor_k in candidate_anchors:  # chain(asym) level
            ...
            # [Xujun] START 确保被mask的chain不会被permutation
            ...
            # [Xujun] START 确保被mask的chain不会被permutation
            candidate_gt_asym_id = [k for k in self.label_asym_dict if k != gt_anchor and k not in self.masked_asym_ids]
            # [Xujun] END
