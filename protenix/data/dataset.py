...
from protenix.utils.mask_generator import MaskGenerator
...
class BaseSingleDataset(Dataset):
    ...
    def read_indices_list(self, indices_fpath: Union[str, Path]) -> pd.DataFrame:
        ...
        # [Xujun] START 如果生成蛋白质主链，则过滤掉配体和核酸
        # if self.data_condition:
        #     EXCLUDED_TYPES = {"ligand", "nuc", "ions", "excluded_ligand", "glycans"}
        #     indices_list = indices_list[(indices_list.mol_1_type == "prot") & (~indices_list.mol_2_type.isin(EXCLUDED_TYPES))]
        # [Xujun] END
    ...
    def _shuffle_array_based_on_mol_id(token_array: TokenArray, atom_array: AtomArray):
        ...
        token_array, atom_array, _, _, _ = CropData.select_by_token_indices(
    ...
    def process_one(
        ...
        # [Xujun] START 
        # 创建backbone subsample的mask
        # mask方法包括了三种：entity, token, atom, all
        # entity: 按照链为单位，保留其backbone atoms，mask其余的侧链原子
        # token: 按照token为单位，保留其backbone atoms，mask其余的侧链原子
        # atom: 整个序列都进行mask，保留其backbone atoms，并额外保留部分原子作为condition，mask其余的侧链原子
        # all: 整个序列都进行mask，只保留backbone atoms
        # condition_atom_mask: 保留的condition原子，包含了condition token的所有原子，以及condition token的backbone原子和tip原子
        # condition_token_mask: 保留的condition token，包含了condition token的所有原子
        masked_asym_ids = []
        if self.data_condition:
            mask_generator = MaskGenerator(cropped_token_array, cropped_atom_array)
            cropped_atom_array, cropped_token_array = mask_generator.apply_mask(data_condition=self.data_condition)
            del mask_generator
        else:
            cropped_atom_array.set_annotation('condition_atom_mask', np.ones(len(cropped_atom_array), dtype=bool))
            cropped_atom_array.set_annotation('condition_token_mask', np.ones(len(cropped_atom_array), dtype=bool))
        # [Xujun] END
        ...
        feat, label, label_full = self.get_feature_and_label(
            ...
            # [Xujun] START 在eval模式下，使用cropped_atom_array，在train模式下，使用bioassembly_dict["atom_array"]
            # 这么做的原因是
            # 函数中的逻辑：full_atom_array根据cropped_atom_array的entity_id和mol_atom_index进行crop，cropped_atom_array是crop后的full_atom_array的子集
            # 在train模式下，没有要求full_atom_array和cropped_atom_array维度一致，因此维度检查不会报错（permutation也不会报错，因为原子是冗余的）
            # 在eval模式下，full_atom_array并没有crop到cropped_atom_array，即full_atom_array和cropped_atom_array是相同的
            # 因此，原代码对坐标进行了维度检查，要求full_atom_array和cropped_atom_array维度一致
            # 但是修改后的代码中，cropped_atom_array部分侧链原子被mask了，如果给full_atom_array，会导致对应回去的atom_array维度高于cropped_atom_array，两者维度不一致
            # 因此，需要给cropped_atom_array作为full_atom_array
    ...
    def get_feature_and_label(
        ...
        Args:
            ...
            # [Xujun] START 增加data_condition and masked_asym_ids
            data_condition: Flag indicating whether to only keep protein backbone atoms, by default False.
            masked_asym_ids: List of masked asym ids.
            # [Xujun] END
        Returns:
        ...
        # [Xujun] START 增加masked_asym_ids，在计算chain permutation时，去除masked的chain
        features_dict["masked_asym_ids"] = masked_asym_ids
        # [Xujun] END
    ...
    def _get_dataset_param(config_dict, dataset_name: str, stage: str):
        ...
        return {
            ...
            **config_dict["base_info"],
            ...
            # [Xujun] START 增加蛋白质主链生成的开关逻辑参数    
            "data_condition": config_dict.get("data_condition", False),
            # [Xujun] END
        }
