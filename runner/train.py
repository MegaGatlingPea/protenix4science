...
class AF3Trainer(object):
    ...
    def try_load_checkpoint(self):
        ...
        def _load_checkpoint(
            ...
            # [Xujun] START 处理checkpoint中参数shape与当前模型不匹配的情况
            ...
            for k, v in checkpoint["model"].items():
                if k in model_state_dict:
                    if v.shape == model_state_dict[k].shape:
                        ...
                        self.print(f"Skipping parameter {k} due to shape mismatch: checkpoint {v.shape} vs model {model_state_dict[k].shape}")
                else:
                    if self.configs.load_strict:
                        filtered_state_dict[k] = v
                    else:
                        self.print(f"Skipping unexpected parameter {k}")
            
            self.model.load_state_dict(
                state_dict=filtered_state_dict,
                strict=self.configs.load_strict,
            )
            # [Xujun] END 处理checkpoint中参数shape与当前模型不匹配的情况
        ...
        for test_name, test_dl in self.test_dls.items():
            ...
            for index, batch in enumerate(tqdm(test_dl)):
                ...
                with enable_amp:
                    ...
                    if self.configs.use_wandb and DIST_WRAPPER.rank == 0:
                        wandb.log(loss_dict, step=index)
                    # lDDT metrics
                    ...
                    # [Xujun] Start dump batch to pkl
                    with open(f"{self.dump_dir}/{pid}.pkl", "wb") as f:
                        pickle.dump(batch, f)
                    # [Xujun] End dump batch to pkl
