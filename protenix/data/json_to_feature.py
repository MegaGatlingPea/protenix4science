...
class SampleDictToFeatures:
    def __init__(self, single_sample_dict, data_condition='diffusion'):
    ...
    # [Zichang] START generate condition mask from additional info
    @staticmethod
    def add_condition_mask(atom_array: AtomArray, metadata) -> AtomArray:
        if metadata is None:
            atom_array.set_annotation("condition_atom_mask", np.ones(len(atom_array), dtype=bool))
            atom_array.set_annotation("condition_token_mask", np.ones(len(atom_array), dtype=bool))
        else:
            ...
            for period in metadata['fixed']:
                if period[0].isalpha():
                    ...
                    start_res_id = period.split('-')[0][1:]
                    end_res_id = period.split('-')[1]
                    for i in range(int(start_res_id), int(end_res_id) + 1):
                        fixed_res.append(f"{chain_id}{i}")
            fixed_atom_residues = metadata['fixed_atom'].keys()
            for res_info in fixed_res:
                chain_id = res_info[0]
                res_id = res_info[1:]
                res_mask = (atom_array.chain_id == chain_id) & (atom_array.res_id == int(res_id))
                if res_info not in fixed_atom_residues:
                    token_mask[res_mask] = True
                else:
                    fixed_atom_list = metadata['fixed_atom'][res_info]
                    for atom_name in fixed_atom_list:
    ...
    def get_atom_array(self) -> AtomArray:
        ...
        # [Zichang] START add condition mask
        atom_array = self.add_condition_mask(atom_array, self.metadata)
        # [Zichang] END
        return atom_array
    ...
    def get_feature_dict(self) -> tuple[dict[str, torch.Tensor], AtomArray, TokenArray]:
        ...
        # [Zichang] START Pop 'condition_atom_mask' key and get its value, need modification
        #if self.condition_atom_mask is not None:
        #    atom_array.set_annotation("condition_atom_mask", np.ones(len(atom_array), dtype=bool))
        #else:
        #    atom_array.set_annotation("condition_atom_mask", np.ones(len(atom_array), dtype=bool))
        #if self.condition_token_mask is not None:
        #    atom_array.set_annotation("condition_atom_mask", np.ones(len(atom_array), dtype=bool))
        #else:
        #    atom_array.set_annotation("condition_token_mask", np.ones(len(atom_array), dtype=bool))
        # [Zichang] END
        ...
        featurizer = Featurizer(token_array, atom_array, data_condition=self.data_condition)
