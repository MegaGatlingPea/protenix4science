...
def centre_random_augmentation(
    ...
    Args:
        ...
        dtype (torch.dtype, optional): the dtype of the output
    Returns:
    ...
    # [Xujun] start 转换到指定类型
    return x_augment_coords.to(dtype), trans_random.to(dtype), rot_matrix_random.to(dtype), x_center.to(dtype)
    # [Xujun] end
...
# [Xujun] Start 定义逆向变化的函数
def reverse_centre_random_augmentation(x_augment_coords: torch.Tensor, trans: torch.Tensor, rot: torch.Tensor, x_center: torch.Tensor) -> torch.Tensor:
    """
    Reverse the centre random augmentation.
    
    Args:
        x_augment_coords (torch.Tensor): augmented coordinates to reverse
            [..., N_sample, N_atom, 3]
        x_center (torch.Tensor): the center of the coords
            [..., N_atom, 3]
    ...
    Returns:
        torch.Tensor: the reversed coordinates
            [..., N_atom, 3]
    """
    N_atom = x_augment_coords.size(-2)

    # 1. Apply inverse translation (subtract the translation vector)
    x_reversed = x_augment_coords - trans[..., None, :]  # [..., N_sample, N_atom, 3]
